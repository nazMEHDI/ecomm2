...
class ListController extends MelisAbstractActionController
{
    ...
    public function renderToolContentAction()
    {
        ...
        $melisTool->setMelisToolKey('users', 'users_tools');

        $columns = $melisTool->getColumns();
        $translator = $this->getServiceManager()->get('translator');
        $columns['actions'] = ['text' => $translator->translate('tr_users_common_table_column_action')];

        $view->tableColumns = $columns;
        $view->toolTable = $melisTool->getDataTableConfiguration('#usersTableContent', true, false, ['order' => [[ 0, 'desc' ]]]);
    ...
    public function getListAction()
    {
        ...
        if($this->getRequest()->isPost()){
            ...
            $tableData = $usersService->getList($start, $length, $melisTool->getSearchableColumns(), $search, $selCol, $sortOrder, $langId)->toArray();
            $dataCount = $usersService->getList(null, null, $melisTool->getSearchableColumns(), $search, null, 'ASC', $langId, true)->current();
    ...
    {
        $this->getEventManager()->trigger('users_delete_end', $this, $this->getRequest());

        return new JsonModel([
            'success' => true,
            'textTitle' => 'tr_users_delete_item',
            'textMessage' => 'tr_users_delete_success',
        ]);
    }
}